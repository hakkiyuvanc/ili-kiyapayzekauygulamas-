...
class DatabaseMonitor:
    ...
    def check_connection_pool(self):
        ...
        logger.info("\nConfiguration:")
        logger.info(
            f"Max pool size: {engine.pool._pool.maxsize if hasattr(engine.pool, '_pool') else 'N/A'}"
        )
        logger.info(
            f"Max overflow: {engine.pool._max_overflow if hasattr(engine.pool, '_max_overflow') else 'N/A'}"
        )
    ...
    def analyze_query_performance(self):
        ...
        queries = [
            ...
            (
                ...
                "SELECT * FROM analyses WHERE user_id = :user_id ORDER BY created_at DESC LIMIT 10",
                {"user_id": 1},
            ),
