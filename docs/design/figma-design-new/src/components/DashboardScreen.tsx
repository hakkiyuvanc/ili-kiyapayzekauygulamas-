    ...
    <div className="bg-white rounded-3xl shadow-2xl p-6 max-h-[90vh] overflow-y-auto">
        ...
        {recentAnalyses.length > 0 ? (
          <div className="space-y-2">
            ...
            {!isPro && recentAnalyses.length > 2 && (
              ...
              >
                +{recentAnalyses.length - 2} analiz daha · Pro'ya geçerek tümünü
                gör
              </button>
            )}
          </div>
      ...
      <div className="p-4 bg-gradient-to-br from-indigo-50 to-purple-50 rounded-2xl border border-indigo-100">
        ...
        <p className="text-xs text-gray-600">
          Sağlıklı ilişkilerde çiftler, sorunların %70'ini çözemez - bunları
          yönetmeyi öğrenir.
        </p>
      </div>
    </div>
  ...
  return (
    ...
    >
      <div className="flex items-start justify-between mb-2">
        ...
        <div
          className={`px-2 py-1 rounded-lg text-xs ${getScoreColor(analysis.score)}`}
        >
          {analysis.score}/100
        </div>
      </div>
