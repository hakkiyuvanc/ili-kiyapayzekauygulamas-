...
export function ResultScreen({ answers, onRestart }: ResultScreenProps) {
  ...
  const calculateScore = () => {
    ...
    answers.forEach((answer) => {
      if (
        answer.answer.includes("Çok") ||
        answer.answer.includes("Tamamen") ||
        answer.answer.includes("Her")
      ) {
        score += 4;
      } else if (
        answer.answer.includes("Genelde") ||
        answer.answer.includes("iyi") ||
        answer.answer.includes("Mutluyum")
      ) {
        score += 3;
      } else if (
        answer.answer.includes("Bazen") ||
        answer.answer.includes("Orta") ||
        answer.answer.includes("Kısmen")
      ) {
        score += 2;
      } else {
        score += 1;
  ...
  return (
    <div className="bg-white rounded-3xl shadow-2xl p-6 overflow-y-auto max-h-[90vh]">
      ...
      <div className="flex flex-col items-center mb-6">
        <div className="relative w-40 h-40">
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center">
            ...
            <span className="text-sm text-gray-500">{getScoreText()}</span>
          </div>
        </div>
        <h2 className="mt-4 text-center">İlişki Sağlığı Skorunuz</h2>
      </div>
      ...
      <div className="space-y-3">
        <button
          onClick={onRestart}
          className="w-full bg-gradient-to-r from-pink-500 to-purple-600 text-white py-4 rounded-2xl hover:shadow-lg transition-all duration-300 flex items-center justify-center gap-2"
        >
          <RefreshCw className="w-5 h-5" />
          Yeni Analiz Yap
        </button>

        <button className="w-full border-2 border-purple-200 text-purple-600 py-4 rounded-2xl hover:bg-purple-50 transition-all duration-300 flex items-center justify-center gap-2">
      ...
      <p className="text-xs text-gray-400 mt-6 text-center">
        Bu analiz genel bir değerlendirmedir. Profesyonel destek için uzman
