...
def auth_headers(client, test_user):
    ...
    response = client.post(
        "/api/auth/login",
        data={"username": test_user.email, "password": "password123"},  # OAuth2 uses 'username'
    )
...
class TestAuthEndpointsIntegration:
    ...
    def test_register_duplicate_email(self, client, test_user):
        ...
        response = client.post(
            "/api/auth/register",
            json={"email": test_user.email, "password": "pass123", "full_name": "Duplicate"},
        )
    ...
    def test_login_success(self, client, test_user):
        ...
        response = client.post(
            "/api/auth/login", data={"username": test_user.email, "password": "password123"}
        )
    ...
    def test_login_wrong_password(self, client, test_user):
        ...
        response = client.post(
            "/api/auth/login", data={"username": test_user.email, "password": "wrongpassword"}
        )
...
class TestFeedbackEndpointsIntegration:
    ...
    def test_create_feedback_invalid_rating(self, client, auth_headers):
        ...
        response = client.post(
            "/api/feedback/",
            headers=auth_headers,
            json={"rating": 6, "comment": "Test"},  # Invalid (max is 5)
        )
    ...
    def test_get_my_feedback(self, client, auth_headers):
        ...
        client.post("/api/feedback/", headers=auth_headers, json={"rating": 5, "comment": "Test 1"})
        client.post("/api/feedback/", headers=auth_headers, json={"rating": 4, "comment": "Test 2"})

        # Get feedback
        response = client.get("/api/feedback/my-feedback", headers=auth_headers)
    ...
    def test_get_feedback_stats(self, client, auth_headers):
        ...
        for rating in [3, 4, 5]:
            client.post("/api/feedback/", headers=auth_headers, json={"rating": rating})

        # Get stats
        response = client.get("/api/feedback/stats", headers=auth_headers)
...
class TestRepositoryPatternIntegration:
    ...
    def test_feedback_crud_flow(self, client, auth_headers):
        ...
        create_response = client.post(
            "/api/feedback/",
            headers=auth_headers,
            json={"rating": 5, "comment": "Integration test feedback", "category": "feature"},
        )
        assert create_response.status_code == 201
        feedback_id = create_response.json()["id"]

        # READ (via my-feedback)
        read_response = client.get("/api/feedback/my-feedback", headers=auth_headers)
        assert read_response.status_code == 200
        feedbacks = read_response.json()
        assert any(f["id"] == feedback_id for f in feedbacks)

        # Stats check
        stats_response = client.get("/api/feedback/stats", headers=auth_headers)
