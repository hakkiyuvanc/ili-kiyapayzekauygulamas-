...
class FileValidator:
    ...
    def validate_file(file: UploadFile) -> tuple[bool, Optional[str]]:
        ...
        if ext not in FileValidator.ALLOWED_EXTENSIONS:
            return (
                False,
                f"Desteklenmeyen dosya formatı: {ext}. İzin verilenler: {', '.join(FileValidator.ALLOWED_EXTENSIONS)}",
            )
    ...
    def extract_from_zip(content: bytes) -> str:
        ...
        try:
            ...
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST, detail="Geçersiz ZIP dosyası"
            )
        except Exception as e:
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
