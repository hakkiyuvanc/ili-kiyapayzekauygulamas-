...
from app.models.database import Analysis, ChatMessage, ChatSession, User
...
def create_session(
    ...
    if not current_user.is_pro:
        ...
        daily_count = (
            ...
            .filter(
                ChatSession.user_id == current_user.id, func.date(ChatSession.created_at) == today
            )
            .count()
        )
    ...
    if session_in.analysis_id:
        analysis = db.query(Analysis).filter(Analysis.id == session_in.analysis_id).first()
        if not analysis:
...
def send_message(
    ...
    # Save user message
    user_msg = ChatMessage(session_id=session.id, role="user", content=message_in.content)
    db.add(user_msg)
    db.commit()
