...
from backend.app.models.database import DailyPulse, User
...
def get_daily_status(
    ...
    pulse = db.query(DailyPulse).filter(
...
def submit_checkin(
    ...
    if existing:
        raise HTTPException(
            status_code=400, 
            detail="Bugünkü kontrol zaten tamamlandı."
        )
...
def get_history(
    ...
    pulses = db.query(DailyPulse).filter(
        DailyPulse.user_id == current_user.id
