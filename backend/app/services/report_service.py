...
class ReportService:
    ...
    def generate_pdf_report(self, analysis_data: Dict[str, Any], user_name: str = "Değerli Kullanıcımız") -> bytes:
        """
        ...
        elements.append(Paragraph(f"Hazırlayan: AMOR AI | Tarih: {datetime.now().strftime('%d.%m.%Y')}", subtitle_style))
        elements.append(Paragraph(f"Sayın {user_name},", normal_style))
        elements.append(Spacer(1, 12))
        elements.append(Paragraph("İlişkinizin dijital röntgenini çektik ve yapay zeka destekli analiz sonuçlarınız aşağıdadır.", normal_style))
        ...
        data = [
            ...
            ["Genel Skor", f"{analysis_data.get('score', 0)}/100", self._get_score_label(analysis_data.get('score', 0))],
            ...
            ["Birliktelik Dili", f"{metrics.get('we_language', {}).get('score', 0)}/100", "Biz Odaklılık"],
        ]
        ...
        elements.append(Paragraph("Bu rapor AMOR Yapay Zeka servisi tarafından oluşturulmuştur. Profesyonel psikolojik tavsiye niteliği taşımaz.", styles['Italic']))
