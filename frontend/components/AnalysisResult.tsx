...
export default function AnalysisResult({ result }: AnalysisResultProps) {
  ...
  const analysisId =
    "analysis_id" in result ? (result as any).analysis_id : null;
  ...
  const handleChatWithCoach = async () => {
    ...
    try {
      const analysisId =
        "analysis_id" in result ? result.analysis_id : undefined;
  ...
  const handleDownloadBackendPdf = async () => {
    ...
    try {
      ...
      a.href = url;
      a.download = `AMOR_Report_${analysisId}.pdf`;
      a.click();
  ...
  const handleDownloadHtml = async () => {
    ...
    try {
      const res = await analysisApi.exportHtml(analysisId);
      const url = URL.createObjectURL(
        new Blob([res.data], { type: "text/html" }),
      );
      const a = document.createElement("a");
      a.href = url;
      a.download = `AMOR_Report_${analysisId}.html`;
      a.click();
      URL.revokeObjectURL(url);
    } catch (error) {
  ...
  const chartMetrics = isV2(result)
    ? {
        sevgi_haritalari:
          result.gottman_report.gottman_bilesenleri.sevgi_haritalari.skor,
        hayranlik_paylasimi:
          result.gottman_report.gottman_bilesenleri.hayranlik_paylasimi.skor,
        yakinlasma_cabalari:
          result.gottman_report.gottman_bilesenleri.yakinlasma_cabalari.skor,
        olumlu_perspektif:
          result.gottman_report.gottman_bilesenleri.olumlu_perspektif.skor,
        catisma_yonetimi:
          result.gottman_report.gottman_bilesenleri.catisma_yonetimi.skor,
        hayat_hayalleri:
          result.gottman_report.gottman_bilesenleri.hayat_hayalleri.skor,
  ...
  return (
    <div
      id="analysis-report"
      className="space-y-6 safe-bottom px-4 bg-romantic-gradient-soft min-h-screen py-8"
    >
      {/* Header */}
      ...
      <div className="ios-card-elevated animate-slideUp">
        <div className="p-6">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-xl font-bold text-[#331A1A]">
              İlişki Sağlığı 💗
            </h2>
            <div className="flex items-center gap-2">
              {/* Client-side screenshot PDF */}
              <button
                onClick={handleDownload}
                disabled={isDownloading}
                title="Ekran görüntüsü PDF"
                className="p-2 bg-[#FFF0F5] hover:bg-[#FFB6C1]/30 rounded-xl transition-colors disabled:opacity-50"
              >
                <Download
                  className={`w-4 h-4 text-[#B76E79] ${isDownloading ? "animate-pulse" : ""}`}
                />
              </button>
              {/* Backend PDF (with psychology data) */}
              {analysisId && (
                <button
                  onClick={handleDownloadBackendPdf}
                  disabled={isDownloadingPdf}
                  title="PDF Rapor İndir (Psikoloji dahil)"
                  id="download-pdf-btn"
                  className="flex items-center gap-1.5 px-3 py-2 bg-[#B76E79] text-white text-xs font-semibold rounded-xl hover:bg-[#9d5a65] transition-colors disabled:opacity-50 active:scale-95"
                >
                  <FileText
                    className={`w-3.5 h-3.5 ${isDownloadingPdf ? "animate-pulse" : ""}`}
                  />
                  {isDownloadingPdf ? "İndiriliyor..." : "PDF"}
                </button>
              )}
              {/* Backend HTML */}
              {analysisId && (
                <button
                  ...
                  title="HTML Rapor İndir"
                  id="download-html-btn"
                  className="flex items-center gap-1.5 px-3 py-2 bg-[#FFF0F5] text-[#B76E79] text-xs font-semibold rounded-xl border border-[#FFB6C1]/40 hover:bg-[#FFB6C1]/20 transition-colors disabled:opacity-50 active:scale-95"
                >
                  <Globe
                    className={`w-3.5 h-3.5 ${isDownloadingHtml ? "animate-pulse" : ""}`}
                  />
                  {isDownloadingHtml ? "İndiriliyor..." : "HTML"}
                </button>
              )}
            </div>
          </div>
          ...
          <div className="flex flex-col items-center relative">
            ...
            <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 text-center">
              ...
              <div className="text-[#6B3F3F] text-xs mt-1 font-semibold">
                {riskLevel}
              </div>
            </div>
          </div>
        ...
        <>
            ...
            <GlassCard className="p-6">
              ...
              <MultiCircularProgress
                ...
                metrics={[
                  {
                    label: "Yakınlık",
                    value: result.gottman_report.duygusal_analiz.yakinlik || 0,
                    color: "#F472B6",
                  },
                  {
                    label: "Empati",
                    value:
                      result.gottman_report.duygusal_analiz.empati_puani || 0,
                    color: "#A78BFA",
                  },
                  {
                    label: "Pozitiflik",
                    value:
                      100 -
                      (result.gottman_report.duygusal_analiz
                        .toksisite_seviyesi || 0),
                    color: "#34D399",
                  },
                  {
                    label: "İletişim",
                    value:
                      result.gottman_report.gottman_bilesenleri.sevgi_haritalari
                        .skor || 0,
                    color: "#60A5FA",
                  },
                ]}
          ...
          <div className="mb-6 animate-fadeIn delay-200">
            ...
            <StreamingInsightsList
              insights={result.gottman_report.tespit_edilen_kaliplar.map(
                (p) => ({
          ...
          <div className="mb-6 animate-fadeIn delay-300">
            ...
            <div className="space-y-3">
              {result.gottman_report.aksiyon_onerileri.map((action, idx) => (
                <div
                  key={idx}
                  className="bg-white p-4 rounded-xl border-l-4 border-l-green-400 shadow-sm"
                >
      ...
      <GlassCard className="bg-gradient-to-r from-rose-400 to-pink-500 text-white border-none animate-fadeIn delay-500">
        <div className="p-4 flex flex-col items-center text-center">
          ...
          <p className="text-white/80 text-sm mb-3">
            Bu rapor hakkında daha fazla detay öğrenmek ister misin?
          </p>
          <button
